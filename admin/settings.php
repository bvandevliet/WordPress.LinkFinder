    ...
    function()
    {
      ...
      <p>
        ...
        <?php _e(
          ...
          <strong>Please note that sometimes links may appear false-positive, always follow the link manually to confirm before making changes!</strong>', 'linkfinder'
        ); ?>
      </p>
 ...
 * @since 2020.06.11
 */
add_action( 'wp_ajax_linkfinder_process_links', function()
{
  if(
    ! wp_doing_ajax() ||
    ! is_user_logged_in()
  ...
  // die();
  wp_die( '', '', array( 'response' => $response_code, 'exit' => true ) );
} );
 ...
 */
add_filter( 'linkfinder_submit_button', function()
{
...
function linkfinder_after_page_submit_cb()
{
  ...
  if(
    ...
    $success_SELFPINGS = linkfinder_manage_site_hyperlinks::allow_selfpings( ! empty( $_POST['avoid_self_pings'] ) ? false : true );
  }
